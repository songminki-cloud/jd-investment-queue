...
우측 상단 `새로고침` 버튼은 `investment-queue.json`과 `market-prices.json`을 캐시 없이 다시 읽습니다. `market-prices.json`은 GitHub Action이 10분마다 자동 갱신합니다.

주의: GitHub Action cron은 정확히 10분마다 보장되는 실시간 타이머가 아닙니다. GitHub 실행 대기 상황에 따라 몇 분 지연될 수 있습니다. 이 보드는 매매 실행 단말이 아니라 판단 큐이므로 화면에는 `가격 기준 HH:mm`으로 기준 시각을 명확히 표시합니다.

수동 갱신:

```sh
node scripts/update-market-prices.mjs
jq empty market-prices.json
git add market-prices.json
git commit -m "Update manual market prices"
git push origin main
```

브라우저에서 GitHub Pages 정적 화면이 네이버증권을 직접 `fetch`하는 방식은 CORS 때문에 안정적으로 쓸 수 없습니다. 따라서 현재 MVP는 GitHub Action이 `market-prices.json`을 갱신하고, 화면은 그 JSON을 읽는 구조로 운영합니다. 진짜 실시간 시세가 필요해지면 별도 서버 프록시나 공식 시세 API를 붙입니다.
