...
우측 상단 `새로고침` 버튼은 `investment-queue.json`과 `market-prices.json`을 캐시 없이 다시 읽습니다. 자동 갱신은 하지 않습니다. JD가 필요할 때만 아래 스크립트를 실행해 가격 JSON을 갱신합니다.
...
브라우저에서 GitHub Pages 정적 화면이 네이버증권을 직접 `fetch`하는 방식은 CORS 때문에 안정적으로 쓸 수 없습니다. 따라서 현재 MVP는 수동 스크립트가 `market-prices.json`을 갱신하고, 화면은 그 JSON을 읽는 구조로 운영합니다. GitHub Action이나 서버 프록시는 나중에 자동화할 때만 검토합니다.
