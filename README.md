...
주의: 화면의 개수는 JD의 전체 보유종목+관심종목 수가 아니라 **큐 카드 수**입니다. 여러 개별 종목이나 ETF를 하나의 판단 단위로 묶은 카드가 포함됩니다.
...
`investment-queue.json` 하나를 큐 카드 원장으로 사용합니다. 화면은 이 JSON을 읽어서 렌더링합니다.

이 파일은 전체 보유종목 원장이 아닙니다. 전체 보유 원장은 Obsidian `JD 현재 보유 기준표`가 기준이고, 이 앱은 그중 투자 판단에 필요한 항목을 큐 카드로 보여줍니다.
...
큐 카드는 하나의 투자 판단 단위입니다. 따라서 개별 종목 1개일 수도 있고, 여러 종목/ETF를 묶은 그룹일 수도 있습니다.

예:

- `전력·원전 ETF 묶음`: KODEX AI전력핵심설비, SOL 미국AI전력인프라, ACE 미국SMR원자력TOP10
- `빅테크·AI ETF 묶음`: ACE 미국빅테크TOP7 Plus, KODEX 미국AI테크TOP10
- `비핵심 미국주 묶음`: NFLX, AAPL, SBUX, DIS, NKE, BYND
- `광통신·광부품 관찰군`: COHR, CIEN, LITE, GLW

전체 종목을 모두 낱개 카드로 펼치면 보드가 다시 토스 관심종목처럼 복잡해지므로, MVP에서는 판단 단위별로 묶습니다.
